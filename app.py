...
st.title("✨ Studio Foto AI Pro v3")
st.write("Aplikasi edit foto cerdas. Potongan tajam & presisi!")

# Memuat "Otak" AI secara efisien agar tidak lambat saat tombol diklik
@st.cache_resource
def get_ai_model():
...
def process_remove_bg(image_input):
    ...
    # Jalankan AI dengan model presisi tinggi
    res_bytes = remove(
        img_byte.getvalue(),
        session=get_ai_model(),
        # Mematikan fitur penghalus (matting) yang membuat tepi kemerahan/blur
        alpha_matting=False 
    )
    ...
    if file_tab1:
        ...
        if st.button("🪄 Proses Ganti Latar", type="primary"):
            with st.spinner("AI sedang memotong dengan presisi tingkat tinggi..."):
                ...
                st.download_button("📥 Download Hasil", data=buf1.getvalue(), file_name="hasil_edit_tajam.png", mime="image/png")
